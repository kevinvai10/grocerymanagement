...
class ProductList extends React.Component {
  ...
  calculateCartTotal() {
    ...
    const total = cart.reduce(
      (acc, currval) => acc + currval.price * currval.amount,
      0
    );
    return total;
  }
  ...
  render() {
    const imgSrc =
      'https://icon-library.net/images/cart-icon-png-white/cart-icon-png-white-4.jpg';
    const {
      products,
      search,
      cartTotal,
    ...
    const filteredProducts = isShowCart
      ? cart
      ...
      <div className='product-list'>
        <div>
          ...
          <div className='cart-options'>
            ...
            />
            <button onClick={this.showCartItemsOnly}>
              {isShowCart ? 'View Products' : 'View Cart'}
            </button>
          </div>
        </div>
