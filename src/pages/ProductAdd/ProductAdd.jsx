...
class ProductAdd extends Component {
  ...
  handleStoreChange(event) {
    const selectedStore = this.state.stores.find(
      store => store.store_name === event.target.value
    );
    this.setState({
      product_store: selectedStore.store_name,
      product_storeid: selectedStore.id
    });
  }
  ...
  handleCategoryChange(event) {
    ...
    this.setState({
      ...
      product_categoryid: selectedCategory.id
    });
  }
  ...
  handleSubmit(event) {
    ...
    if (product_store === '--Select--' || !product_store)
      return alert('Select Category Please');
    if (product_category === '--Select--' || !product_category)
      return alert('Select Store Please');
    const newProduct = {
      name: product_name,
      price: product_price,
        ...
        <main className='pa4 black-80'>
          <form className='measure' onSubmit={this.handleSubmit}>
            <fieldset id='sign_in' className='ba b--transparent ph0 mh0'>
              ...
              <div className='mt3'>
                <label className='db fw6 lh-copy f6 required' htmlFor=''>
                  Name of the product
                </label>
                <input
              ...
              <div className='mv3'>
                ...
                >
                  ...
                  {stores &&
                    stores.map(store => (
                      <option key={store.id}>{store.store_name}</option>
                    ))}
                </select>
              </div>
            </fieldset>
